...
| Force-end a live seasonal event, patch event rewards, or reset one player's event progress | `GET/PATCH/POST/DELETE /api/admin/seasonal-events*` | [`docs/season-ops-runbook.md`](./season-ops-runbook.md), [`docs/player-mailbox-compensation.md`](./player-mailbox-compensation.md) |
| Run reconnect or multiplayer governance checks | `npm test -- e2e:multiplayer:smoke`, `npm test -- sync-governance:matrix`, `npm run stress:rooms:reconnect-soak` | [`docs/sync-governance-matrix.md`](./sync-governance-matrix.md), [`docs/reconnect-smoke-gate.md`](./reconnect-smoke-gate.md), [`docs/reconnect-soak-gate.md`](./reconnect-soak-gate.md) |
...
| Review or validate MySQL persistence expectations | `npm run db -- migrate`, `npm run db -- migrate:rollback`, `npm test -- phase1-release-persistence` | [`docs/mysql-persistence.md`](./mysql-persistence.md). The release-facing regression artifact usually lands in `artifacts/release-readiness/phase1-release-persistence-regression-*.json`. |
...
| Review shared client/server contract coverage | `npm test -- contracts`, `npm test -- shared` | [`docs/shared-contract-snapshots.md`](./shared-contract-snapshots.md), [`docs/test-coverage-audit-issue-199.md`](./test-coverage-audit-issue-199.md) |
