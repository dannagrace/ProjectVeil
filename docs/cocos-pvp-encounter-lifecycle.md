...
- 对手摘要必须至少包含：对手英雄/玩家身份、英雄等级、房间态、遭遇会话、当前回合归属或“等待权威恢复”。
...
- Lobby 房间列表至少要直接显示：当前房间是否在探索、PVP 对抗中、等待重连或恢复中，避免只能靠日志判断房间状态。
...
- `npm test -- e2e:multiplayer:smoke` 仍是 PR 级快反馈，不覆盖 reconnect / 结算恢复的所有分支；这些分支继续用 `npm test -- e2e:multiplayer` 下的对应 PvP spec 复核。
