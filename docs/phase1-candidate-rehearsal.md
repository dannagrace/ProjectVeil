...
- the candidate release-readiness snapshot, the staged runtime observability gate and its Markdown companion, the staged runtime SLO summary JSON / Markdown / text trio, the staged H5 candidate smoke report, the staged reconnect soak summary, the staged WeChat candidate summary and its Markdown companion, the runtime observability bundle, raw runtime observability evidence, and gate, the candidate-scoped Cocos RC bundle, plus the same-revision bundle's manifest, manual evidence owner ledger, and release-readiness dashboard restaged at the rehearsal bundle top level, along with the release gate summary and its Markdown companion, the release health summary and its Markdown companion, the CI trend summary and its Markdown companion, the paired Phase 1 release evidence drift gate, the Phase 1 exit audit, the Phase 1 exit-dossier freshness gate, the Phase 1 candidate dossier and its Markdown companion, and the final go/no-go packet as packet-level reviewer checkpoints
- one candidate-level evidence audit plus the dedicated freshness guard, its owner-reminder and freshness-history companions, along with one current release evidence index and its Markdown companion, so reviewers have a front-door into the packet
...
Open `artifacts/release-readiness/phase1-candidate-rehearsal-local/SUMMARY.md` first. Its reviewer front-door section is the canonical packet-level reviewer entrypoint and now foregrounds the release candidate manifest Markdown artifact, the same-candidate evidence audit Markdown, the runtime observability gate Markdown companion, the runtime SLO summary Markdown companion, the WeChat candidate summary Markdown companion, then the manifest JSON companion, the current release evidence index, the current release evidence index Markdown companion, the release gate summary, the release gate summary Markdown companion, the release health summary, the release health summary Markdown companion, the CI trend summary, the CI trend summary Markdown companion, the release readiness snapshot, the runtime observability gate, the runtime SLO summary, the H5 candidate smoke, the reconnect soak summary, the WeChat candidate summary, the runtime observability bundle, the runtime observability evidence, candidate evidence audit, candidate freshness guard, candidate owner reminder, candidate freshness history, restaged release-readiness dashboard, the same-revision evidence bundle manifest, the Phase 1 release evidence drift gate, the Phase 1 exit audit, the Phase 1 exit-dossier freshness gate, the Phase 1 candidate dossier, the Phase 1 candidate dossier Markdown companion, the go/no-go packet, manual evidence owner ledger, Cocos primary journey evidence, the Cocos reconnect replay artifact, the Cocos main-journey replay gate, Cocos primary diagnostics, the candidate revision triage digest, the Cocos RC bundle, and the release PR summary, then records the release gate and release health outcomes for the candidate revision.
