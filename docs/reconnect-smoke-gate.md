...
- 本地 canonical smoke：`npm test -- e2e:smoke`
- 单测例过滤：`npm test -- e2e:smoke -- reconnect-prediction-convergence`
- 覆盖用例：`tests/e2e/reconnect-prediction-convergence.spec.ts`
- 辅助多人 / 结算恢复参考：
  - `npm test -- e2e:multiplayer -- pvp-reconnect-recovery`
  - `npm test -- e2e:multiplayer -- pvp-postbattle-reconnect`
...
## Battle Settlement Ownership

战后结算 / loot / 装备同步的恢复链路按以下分工执行：

- 服务端拥有最终权威：
  - `battle.resolved` 的胜负、奖励、经验、掉落物归属
  - 战后英雄背包 / 装备栏的最终状态
  - reconnect 后返回给客户端的房间快照
- Cocos 客户端只负责恢复呈现：
  - 可以继续显示最近一次本地缓存的结算摘要，避免玩家在恢复期看到“当前没有战斗”的空白状态
  - 必须明确提示“正在恢复 / 已恢复”，并说明最终结果以权威快照为准
  - 不得在本地重复发放奖励、重复写入掉落或猜测装备结果

对战后恢复链路的预期是：

1. 断线后先保留最近一次结算摘要。
2. 若已进入缓存回放 / 快照回补，UI 明确说明当前只是恢复提示。
3. 权威快照返回后，奖励、loot、装备状态收敛到服务端结果，并允许继续后续操作。
