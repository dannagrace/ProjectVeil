...
| `smoke:ci` | smoke | No tracked artifact; this is a workflow smoke that reports pass/fail on stdout/stderr and exit status. |
...
- Purpose: Run the repo's lightweight CI smoke entrypoint so maintainers can quickly confirm the smoke pipeline wiring still executes before spending time on heavier release validation.
- Required inputs:
