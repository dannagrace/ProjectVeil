...
| `release:cocos-rc:bundle` | release | Bundle files under `artifacts/release-readiness/`, including `cocos-rc-reconnect-replay-<candidate>-<short-sha>.json`, `cocos-rc-evidence-bundle-<candidate>-<short-sha>.json`, `cocos-main-journey-replay-gate-<candidate>-<short-sha>.json`, and companion Markdown/checklist/blocker artifacts. |
...
| `release:wechat:assets-hotfix` | release | `codex.wechat.hotfix-manifest.json` in the selected output directory. |
...
| `smoke:ci` | smoke | `artifacts/release-readiness/smoke-ci-<short-sha>.json` |
...
| `validate:k8s-configmap` | validate | No tracked artifact; exits non-zero when placeholder tokens or invalid configmap substitutions are detected. |
...
| `validate:production-env` | validate | No tracked artifact; exits non-zero and prints missing/blank variables when the production environment file is incomplete. |
...
| `validate:season-reward-config` | validate | No tracked artifact; exits non-zero when the season reward config has schema/type errors or overlapping reward thresholds. |
...
- Purpose: Assemble the Cocos RC evidence packet, reconnect replay artifact, checklist, blockers log, sign-off summary, and manifest for one candidate revision.
- Required inputs:
...
- Family: `release`
- Command: `node --import tsx ./scripts/cocos-rc-reconnect-replay.ts`
- Purpose: Replay the Cocos primary-client reconnect contract for one candidate revision, covering stored-token resume plus resume failure -> fresh join fallback.
- Required inputs:
...
- Family: `release`
- Command: `node --import tsx ./scripts/release-wechat-assets-hotfix.ts`
- Purpose: Generate a WeChat CDN hotfix manifest that diffs one exported wechatgame build against a baseline release manifest and summarizes changed files/subpackages for fast asset rollback.
- Required inputs:
...
- Family: `smoke`
- Command: `node ./scripts/smoke-ci.mjs`
...
- Family: `validate`
- Command: `node --import tsx ./scripts/validate-k8s-configmap-placeholders.ts`
...
- Family: `validate`
- Command: `node ./scripts/validate-production-env.mjs`
...
- Family: `validate`
- Command: `node --import tsx ./scripts/validate-season-reward-config.ts`
- Purpose: Validate `configs/season-rewards.json` against the checked-in schema and server semantics before startup, build, or a seasonal reset can proceed.
- Required inputs:
