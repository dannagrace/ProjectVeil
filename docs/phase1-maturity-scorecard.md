...
The shipped config/content pack validates cleanly, and `npm test -- phase1-release-persistence` has completed at least one current regression on the intended storage mode. When the candidate is reviewing the `frontier-basin`, `stonewatch-fork`, `ridgeway-crossing`, or `highland-reach` Phase 1 packs, include `npm test -- phase1-release-persistence:frontier`, `npm test -- phase1-release-persistence:stonewatch`, `npm test -- phase1-release-persistence:ridgeway`, `npm test -- phase1-release-persistence:highland`, or the equivalent `--map-pack` form in the evidence set as well. For release candidates with `VEIL_MYSQL_*` enabled, that means the generated report should show `Storage: mysql` while still proving player/account/world data flows hold.
