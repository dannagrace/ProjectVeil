...
import { cliInvocationForCommand, getCommandRegistry } from "./ops/command-family.ts";
...
const INVENTORY_METADATA: Record<string, InventoryMetadata> = {
  ...
  "release:cocos-rc:bundle": {
    purpose:
      "Assemble the Cocos RC evidence packet, reconnect replay artifact, checklist, blockers log, sign-off summary, and manifest for one candidate revision.",
    requiredInputs: [
    ...
    producedArtifacts: [
      "Bundle files under `artifacts/release-readiness/`, including `cocos-rc-reconnect-replay-<candidate>-<short-sha>.json`, `cocos-rc-evidence-bundle-<candidate>-<short-sha>.json`, `cocos-main-journey-replay-gate-<candidate>-<short-sha>.json`, and companion Markdown/checklist/blocker artifacts.",
    ],
  },
  "release:cocos:rc-reconnect-replay": {
    purpose:
      "Replay the Cocos primary-client reconnect contract for one candidate revision, covering stored-token resume plus resume failure -> fresh join fallback.",
    requiredInputs: [
      "Pass `--candidate`; optionally pin `--output`, `--markdown-output`, `--owner`, and `--server` for candidate-scoped artifact naming and reviewer metadata.",
    ],
    producedArtifacts: [
      "`artifacts/release-readiness/cocos-rc-reconnect-replay-<candidate>-<short-sha>.json`",
      "`artifacts/release-readiness/cocos-rc-reconnect-replay-<candidate>-<short-sha>.md`",
    ],
  },
  ...
  "validate:k8s-configmap": {
    purpose: "Validate that Kubernetes ConfigMap templates do not ship unresolved placeholder values before deployment automation runs.",
    requiredInputs: [
      "Current repo ConfigMap templates and placeholder rules from the checked-in validation script.",
    ],
    producedArtifacts: [
  ...
  "validate:production-env": {
    purpose: "Validate that the production environment file includes every required MySQL backup, auth, rate-limit, and matchmaking key before deployment.",
    requiredInputs: [
      "Default `ops/env/production.env` or an explicit `--env-file <path>` pointing at the production environment file to audit.",
    ],
    producedArtifacts: [
      "No tracked artifact; exits non-zero and prints missing/blank variables when the production environment file is incomplete.",
    ],
  },
...
export function listRelevantPackageScripts(): Array<[string, string]> {
  const registry = getCommandRegistry();

  return (["release", "validate", "smoke"] as const)
    .flatMap((family) =>
      registry[family].commands.map((entry) => [entry.legacyScript, cliInvocationForCommand(family, entry.name)] as [string, string]),
    )
...
export function buildReleaseScriptInventoryEntries(): ReleaseScriptInventoryEntry[] {
  const relevantScripts = listRelevantPackageScripts();
  ...
  const extraMetadata = Object.keys(INVENTORY_METADATA)
    .filter((script) => !relevantScripts.some(([candidate]) => candidate === script))
    .sort((left, right) => left.localeCompare(right));
  ...
  if (missingMetadata.length > 0 || extraMetadata.length > 0) {
    ...
    throw new Error(`release script inventory metadata is out of sync with the CLI registry (${problems.join("; ")})`);
  }
...
export function renderReleaseScriptInventoryMarkdown(entries: ReleaseScriptInventoryEntry[]): string {
  const lines: string[] = [
    ...
    "",
    "Generated from the unified release/validate/smoke CLI registry. Do not edit this file by hand; update `scripts/release-script-inventory.ts` and regenerate it.",
    "",
    `Relevant scripts: ${entries.length}`,
    "",
    "## Scope",
    "",
    "- Includes every registered release/readiness command whose legacy identifier starts with `release:`, `validate:`, or `smoke:`.",
