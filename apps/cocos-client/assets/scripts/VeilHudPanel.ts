...
import type { PlayerReportReason } from "./VeilCocosSession.ts";
...
import {
  HUD_ACCENT as HUD_ACCENT_MODEL,
  buildHudPanelViewModel,
  type HudPanelViewModel,
  type VeilHudPanelOptions,
  type VeilHudRenderState,
  type VeilHudSessionIndicator
} from "./cocos-hud-panel-model.ts";

export type { VeilHudPanelOptions, VeilHudRenderState, VeilHudSessionIndicator } from "./cocos-hud-panel-model.ts";
...
export const HUD_ACCENT = HUD_ACCENT_MODEL;
...
export class VeilHudPanel extends Component {
  ...
  private onEquipItem: VeilHudPanelOptions["onEquipItem"];
  private onUnequipItem: VeilHudPanelOptions["onUnequipItem"];
  ...
  render(state: VeilHudRenderState): void {
    ...
    const model = buildHudPanelViewModel(state, this.onLearnSkill);
    const hero = model.hero;
    const world = model.world;
    const resources = model.resources;
    const skillPanelView = model.skillPanelView;
    const progressMeter = model.progressMeter;
    const equipmentButtons = this.buildEquipmentButtonStates(model.equipmentRows);
    ...
    cursorY = this.renderCardBlock(
      ...
      leftX,
      4,
      Math.max(174, 52 + model.statusLines.length * 16)
    );
    ...
    if (hero) {
      ...
      this.renderHeroMeters(`${CARD_PREFIX}-hero`, hero.move.remaining, hero.move.total, hero.stats.hp, model.maxHpTotal, hero.armyCount);
    ...
    this.renderStatusBadge(`${CARD_PREFIX}-status`, model.statusBadge);
  ...
  private resolvePointerAction(localX: number, localY: number): HudResolvedAction {
    ...
    const model = this.currentState ? buildHudPanelViewModel(this.currentState, this.onLearnSkill) : null;
    const skillActions = model?.skillPanelView.actions ?? [];
  ...
  private renderLearnableSkillButtons(cardName: string, skills: HudPanelViewModel["skillPanelView"]["actions"]): void {
    const cardNode = this.node.getChildByName(cardName);
    if (!cardNode) {
      return;
    }
  ...
  private buildEquipmentButtonStates(rows: HudPanelViewModel["equipmentRows"]): HudEquipmentButtonState[] {
